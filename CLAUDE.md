...
This file provides guidance to Claude Code (claude.ai/code) and other AI coding agents working in this repository. It reflects the **current state** of the prototype, aligned with `docs/ux-spec.md`.
...
- When `window.location.hash` starts with `#/app`, `App.tsx` mounts `src/prototype/` instead and toggles a `prototype-active` class on `<body>`. This is the actual product mockup — a control plane operator UI for managing AI agents, conversations, runs, approvals, and spend.
